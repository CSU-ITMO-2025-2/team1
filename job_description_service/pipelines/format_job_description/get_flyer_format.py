"""
Функция-обертка, которая обрабатывает входной текст и запускает процесс генерации описания вакансии для флаера.
"""
...
from .flyer_llm.flyer import flyer_llm
from .pydantic_models.flyer_format import FlyerFormat

# Логирование
logger = setup_logger(__name__)


async def get_flyer_format(text: str) -> Optional[FlyerFormat]:
    """
    Асинхронно генерирует вакансию в формате флаера из входного текста.
    ...
    try:
        # Запускаем генерацию описания для флаера
        result = await flyer_llm(cleaned_text)
