"""
Функция-обертка, которая обрабатывает входной текст и запускает процесс генерации описания вакансии для социальных сетей.
"""
...
from pipelines.format_job_description.social_media_llm.social_media import social_media_llm

# Логирование
logger = setup_logger(__name__)


async def get_social_media_format(text: str) -> Optional[SocialMediaFormat]:
    """
    ...
    try:
        result = await social_media_llm(cleaned_text)
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        result1 = await get_social_media_format(test_text_1)
        print(f"Результат: {result1}\n")

        # Тест с пробелами
        print("Тест 2: Текст только с пробелами")
        result2 = await get_social_media_format(test_text_2)
        print(f"Результат: {result2}\n")

        # Тест с пустой строкой
        print("Тест 3: Пустая строка")
        result3 = await get_social_media_format(test_text_3)
        print(f"Результат: {result3}\n")

        # Тест с None
        print("Тест 4: None")
        result4 = await get_social_media_format(test_text_4)
