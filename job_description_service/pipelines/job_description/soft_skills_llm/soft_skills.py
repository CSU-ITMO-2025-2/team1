"""
    Функция для создания запроса для генерации софт скилов с структурированным выводом 
...
async def soft_skills_llm(text: str) -> Optional[GeneratedSoftSkills]:
    """
    Создаёт экземпляр LLM, настроенный на структурированный вывод софт скилов по вакансии.
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            structured_llm = get_structured_llm(
                pydantic_model=GeneratedSoftSkills, attempt_number=attempt
            )

            # Формируем промпт
            prompt_input = {"vacancy_description": text}
            messages = await soft_skills_full_prompt.ainvoke(prompt_input)
    ...
    logger.error(f"Все {MAX_ATTEMPTS} попыток спарсить текст провалились: {str(last_exception)}",
        exc_info=True,
    )
    return None
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        result1 = await soft_skills_llm(test_text_1)
        print(f"Результат: {result1}\n")

        # Тест с пробелами
        print("Тест 2: Текст только с пробелами")
        result2 = await soft_skills_llm(test_text_2)
        print(f"Результат: {result2}\n")

        # Тест с пустой строкой
        print("Тест 3: Пустая строка")
        result3 = await soft_skills_llm(test_text_3)
        print(f"Результат: {result3}\n")

        # Тест с None
        print("Тест 4: None")
        result4 = await soft_skills_llm(test_text_4)
        print(f"Результат: {result4}\n")

        # Тест с минимальным текстом
        print("Тест 5: Минимальный текст")
        result5 = await soft_skills_llm(test_text_5)
