...
Этот модуль предоставляет класс RabbitMQClient, который:
1. Устанавливает устойчивое соединение с RabbitMQ
2. Реализует паттерн RPC (Remote Procedure Call) через очереди
3. Обеспечивает надежную доставку сообщений с подтверждением
4. Поддерживает корреляцию запросов и ответов
5. Имеет встроенный механизм таймаутов для предотвращения зависаний

Использование:
    client = RabbitMQClient(rabbit_url)
    await client.connect()
    result = await client.call(payload, queue_name)
"""
...
class RabbitMQClient:
    ...
    def __init__(self, rabbit_url: str):
        ...
        self.logger.info("Инициализация клиента RabbitMQ",
    ...
    async def connect(self):
        ...
        try:
            ...
            self.connection = await aio_pika.connect_robust(
                self.rabbit_url,
                heartbeat=600  # 600 секунд = 10 минут (увеличено с дефолтных 60 секунд)
            )
    ...
    async def on_response(self, message: aio_pika.IncomingMessage):
        ...
        if self.corr_id == message.correlation_id:
            self.logger.debug("Получено ответное сообщение",
    ...
    async def call(
        ...
        self.logger.debug(f"Отправка сообщения в очередь {queue_name}",
        ...
        for _ in range(6000):  # 600 секунд
            if self.response is not None:
                ...
                self.logger.debug("Получен ответ",
                                extra={"correlation_id": self.corr_id})
                return result
            await asyncio.sleep(0.1)

        self.logger.error("Превышено время ожидания ответа от воркера",
                            extra={"correlation_id": self.corr_id})
