"""
Функция для создания запроса для очистки навыков, которые не получится проверить с структурированным выводом
...
async def clean_skills_llm(skills: list) -> Optional[SkillsList]:
    """
    Создаёт экземпляр LLM, настроенный на структурированный вывод навыков без тех, которые не получится проверить.
    ...
    Args:
        skills: Список навыков для извлечения.
    Returns:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            structured_llm = get_structured_llm(
                pydantic_model=SkillsList, attempt_number=attempt
            )

            # Формируем промпт
            prompt_input = {"skills": skills}
            messages = await clean_skills_full_prompt.ainvoke(prompt_input)
